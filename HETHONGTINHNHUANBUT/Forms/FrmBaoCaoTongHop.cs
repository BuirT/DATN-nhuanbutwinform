        ...
        private void btnXuatExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    // 1. In tiêu đề
                    string title = "BÁO CÁO TỔNG HỢP THÁNG " + dtpThang.Value.ToString("MM/yyyy");
                    ws.Cell(1, 1).Value = title;
                    var titleRange = ws.Range(1, 1, 1, dgvTongHop.Columns.Count);
                    ...
                    for (int i = 0; i < dgvTongHop.Columns.Count; i++)
                    {
                        ...
                        cell.Value = dgvTongHop.Columns[i].HeaderText;
                    ...
                    for (int i = 0; i < dgvTongHop.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgvTongHop.Columns.Count; j++)
                        {
                            ...
                            var val = dgvTongHop.Rows[i].Cells[j].Value;
                    ...
                    string tempPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "BaoCaoTongHop_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx");
